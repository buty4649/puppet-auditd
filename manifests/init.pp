...
class auditd (
  ...
  Pattern[/^(RAW|NOLOG)$/] $log_format          = $::auditd::params::log_format,
  String $log_group                             = $::auditd::params::log_group,
  Optional[Pattern[/^(yes|no)$/]] $write_logs   = $::auditd::params::write_logs,
  Integer $priority_boost                       = $::auditd::params::priority_boost,
  Pattern[/^(none|incremental|incremental_async|data|sync)$/] $flush = $::auditd::params::flush,
  Integer $freq                                 = $::auditd::params::freq,
  Integer $num_logs                             = $::auditd::params::num_logs,
  Pattern[/^(lossy|lossless)$/] $disp_qos       = $::auditd::params::disp_qos,
  Stdlib::Absolutepath $dispatcher              = $::auditd::params::dispatcher,
  Pattern[/^(none|hostname|fqd|numeric|user)$/] $name_format = $::auditd::params::name_format,
  String $admin                                 = $::auditd::params::admin,
  Integer $max_log_file                         = $::auditd::params::max_log_file,
  Pattern[/^(ignore|syslog|suspend|rotate|keep_logs)$/] $max_log_file_action = $::auditd::params::max_log_file_action,
  Integer $space_left                           = $::auditd::params::space_left,
  Pattern[/^(ignore|syslog|email|exec|suspend|single|halt)$/] $space_left_action = $::auditd::params::space_left_action,
  String $action_mail_acct                      = $::auditd::params::action_mail_acct,
  Integer $admin_space_left                     = $::auditd::params::admin_space_left,
  Pattern[/^(ignore|syslog|email|exec|suspend|single|halt)$/] $admin_space_left_action = $::auditd::params::admin_space_left_action,
  Pattern[/^(ignore|syslog|exec|suspend|single|halt)$/] $disk_full_action = $::auditd::params::disk_full_action,
  Pattern[/^(ignore|syslog|exec|suspend|single|halt)$/] $disk_error_action = $::auditd::params::disk_error_action,
  Optional[Integer] $tcp_listen_port            = $::auditd::params::tcp_listen_port,
  Integer $tcp_listen_queue                     = $::auditd::params::tcp_listen_queue,
  Optional[Integer] $tcp_max_per_addr           = $::auditd::params::tcp_max_per_addr,
  Optional[Integer] $tcp_client_ports           = $::auditd::params::tcp_client_ports,
  Integer $tcp_client_max_idle                  = $::auditd::params::tcp_client_max_idle,
  Pattern[/^(yes|no)$/] $enable_krb5            = $::auditd::params::enable_krb5,
  ...
  Pattern[/^(ignore|syslog|suspend|single|halt)$/] $audisp_overflow_action = $::auditd::params::audisp_overflow_action,
  ...
  String $audisp_name_format                     = $::auditd::params::audisp_name_format,
